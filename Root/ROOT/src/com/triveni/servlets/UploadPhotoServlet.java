...
public class UploadPhotoServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int result = changeUserDAO.uploadPhotos(photoBean);
		if (result != 0) {
			LoginDAO loginDAO = new LoginDAO();
			ArrayList<PhotoBean> photoBeans = loginDAO.fetchPhotos();
			session.setAttribute("photoDetails", photoBeans);
			response.sendRedirect("userprofile/uploadPhotos.jsp?status=Successful");
		} else {
			response.sendRedirect("userprofile/uploadPhotos.jsp?status=Failure");
		}
	}
